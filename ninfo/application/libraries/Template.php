...
class Template
{
	...
	// FALSE la vue n'est pas mise encache, INT (en secondes)
	 ...
	 * @return	void
	 */
	public function cache_view($lifetime = 5)
	{
		$this->cache_view = 5;
	...
	public function build($view, $data = array(), $return = FALSE)
	{
		...
		{
			$cache = APPPATH . 'cache/views/' . md5($view) . CACHE_EXT;

			if (false === $cached_view = $this->_ci->cache->get($cache))
			{
				$cached_view = $this->_find_view($view, array(), $this->_parser_body_enabled);
				$this->_ci->cache->save($cache, $cached_view, $this->cache_view);
			}

			$this->_body = $cached_view;
		}
